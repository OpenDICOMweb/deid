...
class DeIdentifier {
  ...
  TagElement replaceUid(Dataset ds, int code, Trial trial, [List values]) {
    print('DS: $ds, tag: ${dcm(code)}, values=$values');
    ...
    print(' replaceUidsByCode: tag${dcm(code)} values=$values');
  ...
  TagElement dummy(Dataset ds, int code, Trial trial, [List values ]) =>
      ds.replaceCode(code, values);

  TagElement zeroOrDummy(Dataset ds, int code, Trial trial, [List values ]) {
    //TODO: fix when we know ATypes
    return ds.replaceCode(code, values);
  }

  TagElement clean(Dataset ds, int tag, Trial trial, [List values ]) {
    return ds.replaceCode(tag, values);
  }

  TagElement kZorD(Dataset ds, int tag, Trial trial, [List values ]) {
    //TODO: fix when we know ATypes
    return dummy(ds, tag, trial, values);
  }

  TagElement kXorZ(Dataset ds, int tag, Trial trial, [List values ]) {
    //TODO: fix when we know ATypes
    return zeroOrDummy(ds, tag, trial, values);
  }

  TagElement kXorD(Dataset ds, int tag, Trial trial, [List values ]) {
    //TODO: fix when we know ATypes
    return dummy(ds, tag, trial, values);
  }

  TagElement kXorZorD(Dataset ds, int tag, Trial trial, [List values ]) {
    //TODO: fix when we know ATypes
    return dummy(ds, tag, trial, values);
  }

  TagElement kXorZorU(Dataset ds, int code, Trial trial, [List values ]) {
    //TODO: fix when we know ATypes
