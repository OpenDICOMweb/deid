...
String getMembers(String cName, List<String> fieldTypes,
    List<List<String>> memberData, int index) {
  List<String> members = [];
  ...
  for (int i = 0; i < memberData.length; i++) {
    ...
    members.add('  static const k$keyword =\n'
        '    const $cName("$keyword", $tag, VR.k${vr.id}, "$action");');
  }
  return members.join('\n');
}
...
String getActionTagsList(
    List rows, int tagOffset, int actionOffset, String actionNames) {
  var values = [];
  for (int i = 0; i < rows.length; i++) {
    var row = rows[i];
